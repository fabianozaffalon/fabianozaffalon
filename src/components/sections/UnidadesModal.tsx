...
// Modal com as 3 unidades (Pelotas / Rio Pardo / Broker Nestlé).
// Independente do ConsultorModal.tsx (que mostra só Pelotas + Rio Pardo) —
// use este quando o Broker Nestlé também precisar aparecer como opção.

interface UnidadesModalContextValue {
  abrir: () => void;
  fechar: () => void;
}

const UnidadesModalContext = createContext<UnidadesModalContextValue | null>(
  null
);

export function useUnidadesModal() {
  const ctx = useContext(UnidadesModalContext);
  if (!ctx) {
    throw new Error(
      "useUnidadesModal precisa estar dentro de <UnidadesModalProvider>"
    );
  }
  return ctx;
}

export function UnidadesModalProvider({
  children,
}: {
  ...
  return (
    <UnidadesModalContext.Provider value={{ abrir, fechar }}>
      {children}
      {aberto && <UnidadesModalDialog onClose={fechar} />}
    </UnidadesModalContext.Provider>
  );
}

export function UnidadesButton({
  children,
  className,
}: {
  children: React.ReactNode;
...
function UnidadesModalDialog({ onClose }: { onClose: () => void }) {
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="unidades-modal-title"
