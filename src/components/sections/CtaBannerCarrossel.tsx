...
export function CtaBannerCarrossel({ ofertas }: { ofertas: Oferta[] }) {
  ...
  const handleMouseEnter = () => { pausedRef.current = true; if (timerRef.current) clearTimeout(timerRef.current); };
  const handleMouseLeave = () => { pausedRef.current = false; startTimer(); };

  const prev = () => goTo((indexRef.current - 1 + ofertas.length) % ofertas.length);
        ...
        <div className="mx-auto w-full max-w-[1280px] px-5 md:px-12">
          <div className="rounded-[28px] bg-[#006EB7] px-8 py-10 md:rounded-[40px] md:px-14 md:py-12">
            <div className="flex flex-col gap-4">
              <h2 className="font-black leading-tight text-white" style={{ fontSize: "clamp(1.5rem, 3vw, 2.25rem)" }}>
                Conte com a gente para
                <br />conquistar o seu espaço.
              </h2>
              <p className="text-sm text-white/95">Temos sempre uma grande oportunidade para o seu negócio.</p>
              <div className="flex flex-col gap-3 sm:flex-row">
                <Link href="/contato" className="flex h-[48px] flex-1 items-center justify-center rounded-[10px] bg-white text-sm font-semibold text-[#006EB7] transition-colors hover:bg-[#005a96] hover:text-white">
                  Quero Comprar
                </Link>
                <Link href="/contato" className="flex h-[48px] flex-1 items-center justify-center rounded-[10px] border-2 border-white text-sm font-semibold text-white transition-colors hover:bg-white hover:text-[#006EB7]">
                  Quero Representação
                </Link>
              </div>
            </div>
        ...
        <div className="rounded-[28px] bg-[#006EB7] px-6 py-6 md:rounded-[40px] md:px-8 md:py-8">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:gap-6">
            ...
            <div className="relative md:w-[38%]">
              ...
              {/* Imagem com fade */}
              <div className="relative w-full overflow-hidden rounded-[16px]" style={{ opacity, transition: "opacity 300ms ease-in-out" }}>
                <Image
                  src={oferta.imagem}
                  alt={oferta.titulo}
                  width={1080}
              ...
              {ofertas.length > 1 && (
                <>
                  <button onClick={prev} aria-label="Oferta anterior"
                  ...
                  <button onClick={next} aria-label="Próxima oferta"
              ...
              {ofertas.length > 1 && (
                <div className="mt-3 flex items-center justify-center gap-2">
                  {ofertas.map((_, i) => (
                    <button key={i} onClick={() => goTo(i)} aria-label={`Oferta ${i + 1}`}
            ...
            <div className="flex flex-1 flex-col gap-5">
              <div className="flex flex-col gap-2">
                <h2 className="font-black text-white" style={{ fontSize: "clamp(1.6rem, 2.8vw, 2.4rem)", lineHeight: 1.2 }}>
                  Conte com a gente
                  <br />para conquistar
                  <br />o seu espaço.
                </h2>
                <p className="text-sm leading-snug text-white/95 md:text-base">
                  ...
                  <br />Chame o consultor da sua região e descubra.
                </p>
              </div>
