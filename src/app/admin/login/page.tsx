import { signIn } from "@/auth";

export default async function LoginPage({
  searchParams,
}: {
  searchParams: Promise<{ error?: string }>;
}) {
  const { error } = await searchParams;

  return (
    <div className="flex min-h-screen items-center justify-center bg-[#00497F]">
      <div className="flex flex-col items-center gap-8 rounded-[20px] bg-white p-12 shadow-xl">
        {/* Logo */}
        <img src="/images/logo.png" alt="Fabiano Zaffalon" className="w-48" />

        <div className="flex flex-col items-center gap-2 text-center">
          <h1 className="text-xl font-black text-[#00497F]">
            Painel Administrativo
          </h1>
          <p className="text-sm text-[#595959]">
            Acesso restrito a usuários autorizados
          </p>
        </div>

        {error === "AccessDenied" && (
          <div className="w-full max-w-xs rounded-[8px] bg-red-50 px-4 py-3 text-center text-sm text-red-600">
            Seu e-mail não tem acesso a este painel. Entre em contato com o administrador.
          </div>
        )}

        {/* Botão Google */}
        <form
          action={async () => {
            "use server";
            await signIn("google", { redirectTo: "/admin" });
          }}
        >
          <button
            type="submit"
            className="flex items-center gap-3 rounded-[8px] border-2 border-[#D1D1D1] bg-white px-6 py-3 text-sm font-semibold text-[#595959] transition-colors hover:border-[#006EB7] hover:text-[#006EB7]"
          >
            <svg
              viewBox="0 0 24 24"
              className="h-5 w-5"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                fill="#4285F4"
              />
              <path
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                fill="#34A853"
              />
              <path
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                fill="#FBBC05"
              />
              <path
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                fill="#EA4335"
              />
            </svg>
            Entrar com Google
          </button>
        </form>
      </div>
    </div>
  );
}
