    ...
    <div className="min-h-screen bg-[#F6F6F6]">
      ...
      <main className="mx-auto max-w-[1280px] px-8 py-12">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-black text-[#00497F]">Cases de Crescimento</h1>
          ...
          <table className="w-full">
            <thead className="bg-[#F6F6F6]">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-[#595959]">Tag</th>
            ...
            <tbody className="divide-y divide-[#F6F6F6]">
              {cases.length === 0 ? (
                ...
                cases.map((c) => (
                  <tr key={c.id} className="transition-colors hover:bg-[#F6F6F6]/50">
                    <td className="px-6 py-4">
                      <div className="relative h-12 w-20 overflow-hidden rounded-[6px]">
                        <img src={c.capa} alt={c.titulo} className="h-full w-full object-cover" />
                      </div>
                    </td>
                    <td className="px-6 py-4 max-w-[280px]">
                      <p className="text-sm font-medium text-[#595959] line-clamp-2">{c.titulo}</p>
                      {c.subtitulo && <p className="text-xs text-[#BCBABA] mt-0.5 line-clamp-1">{c.subtitulo}</p>}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-[#595959]">{c.ordem}</td>
                    <td className="px-6 py-4">
                      <span className={
