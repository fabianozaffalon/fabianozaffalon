...
export default function NovoCasePage() {
  ...
  const handleSubmit = async () => {
    if (!form.titulo || !form.slug || !form.conteudo || !capaFile) {
      alert("Preenche título, conteúdo e seleciona a foto capa.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-[#F6F6F6]">
      ...
      <main className="mx-auto max-w-[860px] px-8 py-12">
        <h1 className="mb-2 text-2xl font-black text-[#00497F]">Novo Case</h1>
        <nav className="mb-8 flex items-center gap-2 text-xs text-[#BCBABA]">
          ...
          <span>/</span>
          <span className="text-[#595959]">Novo</span>
        </nav>
        ...
        <div className="flex flex-col gap-6 rounded-[20px] bg-white p-8 shadow-sm">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" value={form.titulo} onChange={handleTitulo}
              placeholder="Ex: Expansão para Rio Pardo"
              className="rounded-[8px] border border-[#D1D1D1] px-4 py-2.5 text-sm outline-none transition-colors focus:border-[#006EB7]" />
          </div>

          {/* Slug */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-[#595959]">Slug* <span className="font-normal text-[#BCBABA]">(gerado automaticamente)</span></label>
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="text" value={form.subtitulo} onChange={(e) => setForm((p) => ({ ...p, subtitulo: e.target.value }))}
              placeholder="Ex: Como construímos presença em nova região"
              className="rounded-[8px] border border-[#D1D1D1] px-4 py-2.5 text-sm outline-none transition-colors focus:border-[#006EB7]" />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea value={form.conteudo} rows={12}
              ...
              placeholder="Texto completo do case..."
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-[#595959]">Data <span className="font-normal text-[#BCBABA]">(opcional, ex: 2024)</span></label>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-[#BCBABA]">{form.ativo ? "Será publicado" : "Salvar como rascunho"}</span>
          </div>
